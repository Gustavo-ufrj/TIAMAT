...
<div class="py-0 px-2">
	<div class=" mb-3">
		<label for="ftamethod" class="form-label">Choose the Input and Output</label>
		<select class="form-control"  name="ftamethod" id="ftamethod" onchange="selectForm(this);">
			<option value="" dafault>Select the input for this Method</option>
			<% for i = 0 to methods.length-1 %>
			  <option value="<%=methods(i).getAttribute("id")%>"><%=methods(i).getAttribute("input_format")%></option>
			<%Next%>
		</select>
		<br>
		<select class="form-control"  name="ftamethod" id="ftamethod" onchange="selectForm(this);">
			<option value="" dafault>Select the output for this Method</option>
			<% for i = 0 to methods.length-1 %>
			  <option value="<%=methods(i).getAttribute("id")%>"><%=methods(i).getAttribute("output_format")%></option>
			<%Next%>
		</select>
	</div>
</div>		
