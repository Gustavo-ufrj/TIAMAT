...
' if rs.eof then																							
 ' response.redirect "manageRoadmap.asp?stepID="+request.querystring("stepID")
' end if		
...
%>										
  ...
  <nav class="navbar fixed-bottom navbar-light bg-light">
    ...
    <%if getStatusStep(request.querystring("stepID")) = STATE_ACTIVE then %>
		<button class="btn btn-sm btn-secondary m-1" type="button" data-bs-toggle="modal" data-bs-target="#manageEvents" data-title="Add Event" data-url="eventActions.asp?action=new&roadmapID=<%=Request.QueryString("stepID")%>" data-event-id="" data-event="" data-year=""> <i class="bi bi-plus-square text-light"></i> Add Event</button>
...
(function() {
		...
		function defineMaxScroll(){
			// Quatro eventos por página, mas tem que tirar um por causa do <li> extra no final.
		  ...
		  function setEqualHeights(el) {
			...
			for (let i = 0; i < el.length; i++) {
			  const singleHeight = el[i].offsetHeight;
				...
				console.log(i, singleHeight, el[i].offsetHeight);
			...
			//Workaround sometimes el won't get offsetHeight, set default to 200
			if (counter == 0) counter = 150;
			
			for (let i = 0; i < el.length; i++) {
			  el[i].style.height = `${counter}px`;
			  console.log(i, el[i].style.height);
			}
		  }
