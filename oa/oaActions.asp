...
Dim usuarios
	...
	case "save"
		...
		if request.form("stepID") <> "" then
			...
			call getRecordSet(SQL_CONSULTA_OA(request.form("stepID")), rs)
			
			if rs.eof then 'new
			
				call ExecuteSQL(SQL_CRIA_OA(request.form("stepID"), request.form("benefit")))
			
			else 'update
			
				call ExecuteSQL(SQL_ATUALIZA_OA(request.form("stepID"), request.form("benefit")))
